...
ForestDiscreteChoice::ForestDiscreteChoice() {
}

ForestDiscreteChoice::~ForestDiscreteChoice() {
}

void ForestDiscreteChoice::loadForest(size_t dependent_varID, size_t num_trees,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    Tree* tree = new TreeDiscreteChoice(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i],
...
void ForestDiscreteChoice::predictInternal() {
    ...
    if (predict_all || prediction_type == TERMINALNODES) {
      ...
      for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
        if (prediction_type == TERMINALNODES) {
          predictions[0][sample_idx][tree_idx] = ((TreeDiscreteChoice*) trees[tree_idx])->getPredictionTerminalNodeID(
              sample_idx);
        } else {
          predictions[0][sample_idx][tree_idx] = ((TreeDiscreteChoice*) trees[tree_idx])->getPrediction(sample_idx);
        }
      }
    } else {
      // Mean over trees
      ...
      for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
        prediction_sum += ((TreeDiscreteChoice*) trees[tree_idx])->getPrediction(sample_idx);
      }
...
void ForestDiscreteChoice::computePredictionErrorInternal() {
...
// For each sample sum over trees where sample is OOB
  ...
  for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
    for (size_t sample_idx = 0; sample_idx < trees[tree_idx]->getNumSamplesOob(); ++sample_idx) {
      ...
      double value = ((TreeDiscreteChoice*) trees[tree_idx])->getPrediction(sample_idx);
...
void ForestDiscreteChoice::writeOutputInternal() {
  *verbose_out << "Tree type:                         " << "DiscreteChoice" << std::endl;
}

void ForestDiscreteChoice::writeConfusionFile() {
...
void ForestDiscreteChoice::writePredictionFile() {
...
void ForestDiscreteChoice::saveToFileInternal(std::ofstream& outfile) {
...
void ForestDiscreteChoice::loadFromFileInternal(std::ifstream& infile) {
...
// Read treetype
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Create tree
    Tree* tree = new TreeDiscreteChoice(child_nodeIDs, split_varIDs, split_values, &is_ordered_variable);
    trees.push_back(tree);
  }
}
